...
async function fetchInitialCPCBCities() {
      ...
      .then(({ cities, meta, broadMeta }) => {
        ...
        if (!backgroundMeta.fallback) {
          ...
          fetchCPCBCities(backgroundOptions)
            .then(({ cities: moreCities, meta: moreMeta }) => {
              if (cancelled || !moreCities?.length) return;

              setCpcbCities(prev => {
                const merged = mergeStations(prev, moreCities);
                cpcbCitiesRef.current = merged;
                setCpcbMeta({ ...moreMeta, totalStations: merged.length, cityCount: merged.length });
                return merged;
              });
            })
            .catch(err => {
